...
class WhitelistModule(torch.nn.Module):
    def __init__(self, dtype):
        ...
        self.weight = torch.nn.Parameter(torch.arange(8*8, device='cuda', dtype=dtype).reshape(8,8).contiguous())
...
class BlacklistModule(torch.nn.Module):
    def __init__(self, dtype):
        ...
        self.weight = torch.nn.Parameter(torch.arange(2*8, device='cuda', dtype=dtype).reshape(2,8).contiguous())
